...
func NewSMD(
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert original object to typed object: %v", err)
	}

	patchObjTyped, err := typeConverter.ObjectToTyped(patch)
	if err != nil {
		return nil, fmt.Errorf("failed to convert patch object to typed object: %v", err)
	}

	newObjTyped, err := liveObjTyped.Merge(patchObjTyped)
	if err != nil {
		return nil, fmt.Errorf("failed to merge patch: %v", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert typed object to object: %v", err)
	}

	return &smdPatchObject{
		patchedObject: newObj,
	}, nil
}
...
func (p *smdPatchObject) GetPatchedObject() runtime.Object {
