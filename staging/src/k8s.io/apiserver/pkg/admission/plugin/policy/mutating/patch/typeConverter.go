...
func (t *typeConverterManager) Run(ctx context.Context) {
	...
	wait.PollUntilContextCancel(ctx, 30, true, func(ctx context.Context) (done bool, err error) {
		paths, err := t.openapiClient.Paths()
		if err != nil {
			utilruntime.HandleError(fmt.Errorf("failed to fetch openapi paths: %v", err))
			return false, nil
		}
		...
		for path, entry := range paths {
			...
			if err != nil {
				utilruntime.HandleError(fmt.Errorf("failed to parse group version %q: %v", path, err))
				return false, nil
			}
...
func (t *typeConverterManager) GetTypeConverter(gvk schema.GroupVersionKind) managedfields.TypeConverter {
	...
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("failed to get schema for %v: %v", gvk, err))
		return nil
	}

	var sch spec3.OpenAPI
	if err := json.Unmarshal(schBytes, &sch); err != nil {
		utilruntime.HandleError(fmt.Errorf("failed to unmarshal schema for %v: %v", gvk, err))
		return nil
	}
	...
	if err != nil {
		utilruntime.HandleError(fmt.Errorf("failed to create type converter for %v: %v", gvk, err))
		return nil
	}
